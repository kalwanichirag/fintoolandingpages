...
const ResetTimer = ({ resendFlag, resetFun }) => {
    ...
    return (
        <>
            {(countdown === 0 || resendFlag) ? (
                <span
                    style={{ color: '#042b62', cursor: 'pointer' }}
                    onClick={() => {
                ...
                <span>Resend in {secondsToMMSS(countdown)}</span>
            )}
        </>
    );
};

function OtpView({ setCurrView, setCurrAppointmentView, formState, sendOtp, setLeadData }) {
    ...
    const handleVerify = async () => {
        ...
        try {
            ...
            // Save lead data via prop callback
            setLeadData(formState);
    ...
    return (
        <div>
            <span
                style={{ position: 'absolute', top: '0', left: '0', margin: '1rem', fontSize: '1.5rem', cursor: 'pointer' }}
                onClick={() => setCurrView('DETAILS')}
            >
                <i className="fa-solid fa-arrow-left"></i>
            </span>

            <div className={Styles.inputDetailsContainer}>
                <div className={Styles.inputElemTitle} style={{ textAlign: 'center', marginBottom: 0 }}>
                    Two-Factor Verification
                </div>
                <div className={Styles.inputElemSubTitle} style={{ textAlign: 'center' }}>
                    Enter the verification code sent to
                </div>
                <div className={Styles.maskedNo} style={{ textAlign: 'center' }}>
                    {maskPhoneNumber(formState.mobile + "")}
                </div>
